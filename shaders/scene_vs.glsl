...
void main()
{
    ...
    positionWorld = model * vec4(vertexPosition, 1.0);
	normalWorld = inverse(transpose(model)) * vec4(vertexNormal, 0);

	// pass the uv coordinate
	uv = vertexUV;

	// normal mapping
	vec3 T = normalize(vec3(inverse(transpose(model)) * vec4(tangent, 0.0)));
	vec3 B = normalize(vec3(inverse(transpose(model)) * vec4(biTangent, 0.0)));
	vec3 N = normalize(vec3(inverse(transpose(model)) * normalWorld));
	TBN = mat3(T, B, N);
}
